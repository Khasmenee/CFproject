...
<body>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
        ...
        <div class="col-md-3">
            <a class="btn btn-primary" href="{{ url('dashboard/order') }}">กลับไปยังคำสั่งซื้อทั้งหมด</a>
        </div>
...
<div class="col-md-7 col-lg-3">
    ...
    <center>
    <br> 
    <form action="{{ url('/status/update/'.$orders[0]->id)}}" method = "POST">
    {{csrf_field()}}
    <div class="col-md-6">
        <select name="status" class="form-control">
                    <option value="1">รอการยืนยัน</option>
                    <option value="2">ยืนยันคำสั่งซื้อ</option>
                    <option value="3">ผลิตสินค้า</option>
                    <option value="4">จัดส่งสินค้า</option>
                    <option value="5">ได้รับสินค้า</option>
         </select>
         </div>
     ...
     <div class="col-md-9">
        <br>
         <button type="button" data-toggle="modal" data-target="#myModal" class="btn btn-danger">ปฏิเสธ</button>
    </div>
            <!-- Modal -->
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">ปฏิเสธคำสั่งซื้อ</h4>
      </div>
      <form action="{{ url('/reject/order/'.$orders[0]->id)}}" method="post">
      		{{csrf_field()}}
	      <div class="modal-body">
				@include('dashboard.status.rejectOrder')
	      </div>
